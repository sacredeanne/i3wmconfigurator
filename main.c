...
void fonts()
{
    ...
    fgets(font_temp, sizeof(font_temp), stdin);
    font_temp[strcspn(font_temp, "\n")] = 0;
    if (strcmp (font_temp, "pango") == 0)
    {
        ...
        scanf("%5s", font_type);
        printf("Is it a mono font? (yes-no): ");
        scanf("%3s", temp_str);
...
{
    int border_size = 0;
    int float_min_size_x = 0, float_min_size_y = 0, float_max_size_x = 0, float_max_size_y = 0;
    int urgency_delay = 0, gaps_size = 0, gaps_outer = 0;
...
void colors()
{
    int bg, fg, border, indicator, child_border;
        int bg_in, fg_in, border_in, indicator_in, child_border_in;
        int bg_un, fg_un, border_un, indicator_un, child_border_un;
        int bg_urg, fg_urg, border_urg, indicator_urg, child_border_urg;
        int bg_ph, fg_ph, child_border_ph;
        int background;
        char temp_str[4];
